...
#                           ----------PROBLEM 2----------
#========================================================================================#  
def build_mlp(input_placeholder, output_size, scope, n_layers=1, size=32, activation=tf.tanh, output_activation=None):
    """
    ...
    with tf.variable_scope(scope):
        for h in range(n_layers):
            input_placeholder=tf.layers.dense(input_placeholder,units=size,activation=activation)
        output_placeholder=tf.layers.dense(input_placeholder,units=output_size,activation=output_activation)
...
class Agent(object):
    ...
    def init_tf_sess(self):
        ...
        # saver.save(self.sess,"./myModel/model")
    ...
    def policy_forward_pass(self, sy_ob_no):
        ...
        if self.discrete:
            # ----------------YOUR_CODE_HERE-----------------------#
            sy_logits_na = build_mlp(sy_ob_no,self.ac_dim,'pi',n_layers=1,size=32)
            return sy_logits_na
        else:
            # ----------------YOUR_CODE_HERE-----------------------#
            sy_mean = build_mlp(sy_ob_no,self.ac_dim,'pi',n_layers=1,size=32)
    ...
    def sample_action(self, policy_parameters):
        ...
        if self.discrete:
            ...
            # ---------------------YOUR_CODE_HERE------------------#
            sy_sampled_ac = sy_mean+tf.random_normal(tf.shape(sy_mean))*tf.exp(sy_logstd)  #易写错为 *sy_logstd 
    ...
    def get_log_prob(self, policy_parameters, sy_ac_na):
        ...
        if self.discrete:
            sy_logits_na = policy_parameters
            # ------------------------YOUR_CODE_HERE-----------------------#
            sy_logprob_n = tf.reduce_sum(tf.one_hot(sy_ac_na,self.ac_dim)*tf.nn.log_softmax(sy_logits_na),axis=1)
        else:
            sy_mean, sy_logstd = policy_parameters
            # ------------------------YOUR_CODE_HERE-----------------------#
            pre_sum = ((sy_ac_na - sy_mean) / tf.exp(sy_logstd)) ** 2 + 2 * sy_logstd + np.log(2 * np.pi)
            sy_logprob_n = -0.5*tf.reduce_sum(pre_sum , axis=1)
        return sy_logprob_n
    ...
    def sum_of_rewards(self, re_n):
        ...
        if self.reward_to_go:
            # raise NotImplementedError
            q_n=[list(accumulate(re[::-1], lambda x,y:x*self.gamma+y))[::-1] for re in re_n]
            q_n=np.concatenate(q_n)
        else:
            # raise NotImplementedError
            # q_n=[np.sum(i)*np.ones_like(i) for i in re_n]
            # q_n=np.concatenate(q_n)
            q_n=[[sum(re)]*len(re) for re in re_n]
            q_n=np.concatenate(q_n)
        return q_n
...
def main():
    ...
    class Args(object):
        ...
        def __init__(self):
            ...
            self.exp_name='ll_b40000_r0.005'
            self.render=False
            self.discount=0.99
            self.n_iter=100
            ...
            self.dont_normalize_advantages=False
